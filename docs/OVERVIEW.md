...
All data calls run through an abstract contract layer. For now the contract is fulfilled by in memory mocks that make the UI feel alive. When Supabase is introduced, only the data source implementation and configuration files should change while UI logic, styling, and contracts remain untouched.

## Design Tenets

- Deliver a premium, high contrast visual language using the Night Navy to Deep Slate palette.
